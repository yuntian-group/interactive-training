...
class DatasetState:
    ...
    interactive_parameters: Dict[str, any] = field(default_factory=dict)
...
class InteractiveServer:
    ...
    def update_dataset_state(self, event: dict):
        """
        Update the dataset state with new parameters.
        This method is thread-safe and updates the dataset state.
        """

        print("Updating dataset state with event:", event)
        ...
        if event["command"] not in {
            ...
            logger.warnig(
                f"Unsupported command for dataset state update: {event['command']}"
            )
            return

        with self._dataset_state_lock:

            if event["command"] == INIITAL_DATASET_INFORMATION:
                new_parameters = json.loads(event.get("args", "{}"))
                initialize_params = new_parameters.get("initialization_parameters", {})
                interactive_params = new_parameters.get("interactive_parameters", {})
                self._dataset_state.initialization_parameters.update(initialize_params)
                self._dataset_state.interactive_parameters.update(interactive_params)

            elif event["command"] == UPDATE_DATASET:
                new_parameters = json.loads(event.get("args", "{}"))
                self._dataset_state.initialization_parameters.update(new_parameters)
                logger.info(
                    f"Updated dataset initialization parameters: {new_parameters}"
                )

            elif event["command"] == UPDATE_DATASET_RUNTIME_HYPERPARAMETERS:
                new_parameters = json.loads(event.get("args", "{}"))
                self._dataset_state.interactive_parameters.update(new_parameters)
                logger.info(f"Updated dataset interactive parameters: {new_parameters}")
    ...
    def update_log_state(self, log: dict):
        with self._log_lock:
            ...
            if "global_step" not in log:
                logger.warning(f"Log does not contain 'global_step': {log}")
    ...
    def fork_branch(
        ...
        with self._log_lock:
            ...
            if new_branch_name in self._logs.branched_logs:
                logger.warning(f"Branch {new_branch_name} already exists, not forking.")
                return {}
            ...
            logger.info(f"Forked new branch: {new_branch_name}")
            return {
                "id": new_branch_name,
    ...
    def update_server_state(self, event: dict):
        ...
        def _update_command_history(event: dict):
            ...
            if cur_uuid is None:
                logger.warning("No UUID found in event, cannot update command history.")
                return
        ...
        if event["command"] in {
            ...
            self.update_dataset_state(event)
            return

        with self._train_state_lock:
            _update_command_history(event)

            if event["status"] != CMD_SUCCESS:
                logger.info(f"Received event with error status: {event}")
                return

            if event["command"] == CHECKPOINT_INFO_UPDATE:
                checkpoint_info_json = event.get("args", None)
                if checkpoint_info_json is None:
                    logger.warning("No checkpoint info provided in event args.")
                    return
                try:
                    checkpoint_info = json.loads(checkpoint_info_json)
                except json.JSONDecodeError:
                    logger.warning(
                        f"Invalid JSON format in event args: {checkpoint_info_json}"
                    )
                    return
    ...
    def _setup_routes(self):
        ...
        async def get_dataset_info():
            ...
            Returns a JSON representation of the DatasetState.
            """
            with self._dataset_state_lock:
                ret = {
                    "initialization_parameters": self._dataset_state.initialization_parameters,
                    "interactive_parameters": self._dataset_state.interactive_parameters,
                }
                ...
                while True:
                    ...
                    for client, result in zip(clients, results):
                        if isinstance(result, Exception):
                            logger.info(f"Error sending to {client.client}: {result}")
                            to_remove.add(client)
                    self._event_listeners -= to_remove

            except WebSocketDisconnect as e:
                logger.info(
                    f"WebSocket disconnected (code: {e.code}): {websocket.client}"
                )
    ...
    def run(self):
        ...
        if self._app_thread is not None and self._app_thread.is_alive():
            logger.warning("Server thread is already alive.")
            return

        self.running = True
        logger.info(
            f"Starting server at {self.host}:{self.port} (timeout={self.timeout}s)."
        )

...
const CheckpointControl: React.FC<{
  ...
  return (
    <div className={clsx("checkpoint-control p-2 h-full flex flex-col", className)}>
      ...
      <div className="checkpoint-list flex flex-col flex-1 min-h-0">
        ...
        <h2 className="text-base font-semibold mb-1 h-6 flex-shrink-0">Current Checkpoint</h2>
            ...
            <div className="space-y-1">
              {checkpointState.state.map((checkpoint) => (
                <SingleCheckpointDisplay
                  ...
                  onLoadCheckpoint={(ckpt) =>
                    console.log("Load Checkpoint:", ckpt)
                  }
                />
              ))}
            </div>
